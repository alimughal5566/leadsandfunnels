...
/**
 * Class HomebotIntegrationSeeder
 *
 * Command to execute seeder - php artisan db:seed --class=HomebotIntegrationSeeder
 */

class HomebotIntegrationSeeder extends Seeder
{
    /**
    ...
    public function run()
    {
        $this->integrationName = config('integrations.iapp.HOMEBOT')['sysname'];
...
//        Global check for entries in client_integrations for Homebot
...
//        if ($count) {
//            echo"#################################################################### \n";
//            echo"Found records against Homebot, looking like seeder already executed \n";
//            echo"#################################################################### \n";
//            return false;
//        }
        ...
        $clients = DB::table('clients')
            ...
            ->join('homebot', function ($join) {
                $join->on('clients.client_id', '=', 'homebot.client_id')
                    ->where('homebot.active', '=', 1);
            })
...
//            ->where("clients.client_id", 3111)
        ...
        foreach ($clients as $client) {
            ...
            if ($count) {
                echo"#################################################################### \n";
                echo"Already found Homebot integrations against this client, ID#" . $client->client_id . " \n";
                echo"#################################################################### \n\n";
                continue;
            }
...
//                dd(DB::getQueryLog(), count($clientLeadpops), count($clientLeadpops1));
            ...
            if(count($clientLeadpops)) {
                echo"Adding Homebot integrations against this client, ID#" . $client->client_id . " \n";

                //Inserting integrations
                $this->insertHomebotIntegrations($client->client_id, $clientLeadpops);
            }
        }
    }


    private function insertHomebotIntegrations($client_id, $clientLeadpops)
    {
        $homebotIntegrations = [];
...
//        DB::enableQueryLog();
        DB::table('client_integrations')->insert($homebotIntegrations);
