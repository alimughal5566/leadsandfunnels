...
var lpUtilities = {
    ...
    * */
    ...
    custom_color_pos: function (pos_top, color_picker_elm, color_picker_block_height, rtl){
        ...
        if(site.route === "funnel-builder" && $elm === '[data-color-dropdown]'){
            scrollHeight = $('.panel-aside-wrap-overlay').find(".mCSB_dragger").position().top;
        }
    ...
    color_picker_dropdown: function () {
        ...
        $('.color-picker-options').on('change', function () {
            ...
            if($this == 1){
                ...
                $(color_picker_elm).parents().find('.color-pull-block').hide();
                $(color_picker_elm).parents().find('.color-picker-block').show();
                $(this).parents('.color-box__panel-wrapper').addClass('own-color-active');
            }else {
                lpUtilities.custom_color_pos(pos_top, $elm, 254, rtl);
                $(color_picker_elm).parents().find('.color-picker-block').hide();
                $(color_picker_elm).parents().find('.color-pull-block').show();
                $(this).parents('.color-box__panel-wrapper').removeClass('own-color-active');
            }
        });
    },
        ...
        $('.quick-scroll').mCustomScrollbar({
            ...
            callbacks:{
                onScrollStart:function(){
                    ...
                    if(jQuery(".create-funnel .modal-content > .select2-container--open").length > 0) {
                        jQuery('.select2-search__field.za-tag-dropdown-modifier').click();
                    }
                },
                onScroll:function(){
    ...
    dropdownpos: function (ele) {
        if($.inArray( ele,['clone-new-tag','funnel-tag-result']) != -1) {
            ...
            if(ele === 'funnel-tag-result' && $(".funnel-empty-tag-list").length === 0) {
                const new_tag_value = $(".create-new-funnel .select2-search input").val();
                if (new_tag_value && new_tag_value.trim() && $(".create-new-funnel .add-tag-wrap").length === 0){
                    $(".select2-results__options").append(`<li role="alert" aria-live="assertive" class="select2-results__option select2-results__message funnel-empty-tag-list d-block"><div class="add-tag-wrap"><a href="#" class="add-tag" data-parent="funnel-tag-result" data-tag="${new_tag_value}"><i class="ico ico-plus"></i><span class="create-new-tag-text">Create new tag</span> <span class="tag-item"><i class="ico ico-tag"></i><span class="terms">${new_tag_value}</span></span></a></div></li>`);
                }
            }
        } else {
    ...
    * */
    ...
    scrollArea: function () {
        ...
        if(jQuery(".conditional-modal-quick-scroll").length > 0) {
            jQuery(".conditional-modal-quick-scroll").mCustomScrollbar({
                axis: "y",
                scrollInertia: 200,
            });
        }
    ...
    * */
    ...
    lpCustomSelect: function () {
        jQuery('.lp-custom-select__opener').click(function(e){
            e.preventDefault();
            var _self = jQuery(this),
                parent = _self.parent('.lp-custom-select');
            parent.addClass('lp-custom-select-active');
            parent.find(".new-tags-holder").removeClass('lp-text-field-active');
            parent.find(".new-tags-holder").find("#new_folder").val("");
        });
        ...
        jQuery('.lp-close-custom-tag').click(function(e){
            e.preventDefault();
            let parent = jQuery(this).parents('.new-tags-holder');
            parent.removeClass('lp-text-field-active');
            parent.find("#new_folder").val("");
        });
        ...
        jQuery(document).on('click', '.lp-add-custom-tag', function(e){
            ...
            $('.lp-custom-select__list__item').each(function (obj,element){
                if($(this).text().toLowerCase() == getVal.toLowerCase()) {
                    $('.lp-custom-select__list__item').eq(obj).click();
                    folder_exist = true;
                    return false;
                }
            });
            ...
            var setVal = '<li class="lp-custom-select__list__item selected folder_added">'+ getVal +'</li>';
    ...
    editorPopupPosition: function(window_height, select_total, select_dropdown, block_height, select_button){
        if (window_height > select_total) {
            $(".fr-popup.fr-active").offset({
                top: select_button.top - select_dropdown + block_height + 20 ,
            });
        } else {
            $(".fr-popup.fr-active").offset({
                top: select_button.top - select_dropdown + block_height + 20 ,
...
jQuery(document).ready(function () {
    ...
    $(document).on('keyup','.select2-search__field.za-tag-dropdown-modifier',function (e){
        $self_value = $(this).val();
        var duplicateTag = tag_dropdown.find(function( idx ) {
            return idx.tag_name.toLowerCase() === $self_value.toLowerCase();
        });
        if(duplicateTag)
        {
            $('.funnel-empty-tag-list').removeClass('d-block').addClass('d-none');
        }else{
            $('.funnel-empty-tag-list').removeClass('d-none').addClass('d-block')
        }
        if (e.keyCode == 13) {
    ...
    $(document).on('keyup', '.select2js__tags-parent .select2-search__field', function(e){
        ...
        if($(this).parents('.create-new-funnel')){
            if(term) {
                let matcher = tag_dropdown.find((e) => e.tag_name === term);
                if (matcher) {
                    $(".funnel-empty-tag-list").hide();
                }
                else{
                    $(".funnel-empty-tag-list").show();
                    // $(".funnel-empty-tag-list .add-tag").attr('data-tag',term);
                    // $(".funnel-empty-tag-list .tag-item .terms").html(term);
                    }
                }
            }
    });
   ...
   // reset the state of stats and conversion rate setting modal on dashboard
    ...
    /* Close Bootstrap Modal on Escape */
    $(document).keydown(function(event) {
        if (event.keyCode == 27) {
            if (!$('body').hasClass('hidden-field-modal')) {
                if( $('body').hasClass('modal-open') && $(".modal.show").attr("data-keyboard")!=='false' )
                    $('.modal').modal('hide');
            }
        }
    });
    ...
    jQuery(".actions-button__link_create-funnels").click( ()=>{
        tag_dropdown = jQuery.parseJSON(tag_list);
    ...
    $('#create-funnel').on('show.bs.modal', function () {

        $("#create-funnel-pop #funnel_name,#create_funnel_tag_list").val('');
        $("#create_funnel_tag_list").val('').trigger("change");
        jQuery('.lp-custom-select .text-selected').html('Select Folder');
        jQuery('.lp-custom-select__opener').removeClass("text-selected");
        jQuery('#create_funnel_folder_id').val('');
        $(".create-new-funnel input#new_folder").val('');
        $(".create-new-funnel .new-tags-holder").removeClass('lp-text-field-active');
    })

    /**
     * Create form submit Disabled on enter key
     */
    $(document).on("keydown", "#create-funnel-pop", function(event) {
        return event.key != "Enter";
    });

    jQuery(document).on('submit',"#create-funnel-pop",function (e)
    {
        $(".create-new-funnel button[type='submit']").prop("disabled",true);
        $(".create-new-funnel #btn-cancel-funnel-pop").prop("disabled",true);
        e.preventDefault();
            ...
            jQuery.ajax({
                ...
                success: function (response) {
                    if(response.redirect !== undefined) {
                        ...
                        $(".create-new-funnel button[type='submit']").prop("disabled",false);
                        $(".create-new-funnel #btn-cancel-funnel-pop").prop("disabled",false);
                    }
                },
                error: function (){
                    $(".create-new-funnel button[type='submit']").prop("disabled",false);
    ...
    /**
     * Add new folder on enter key press
     */
    jQuery(document).on('keyup', '.create-new-funnel #new_folder', function(event){
        if (event.keyCode === 13) {
            // Cancel the default action, if needed
            event.preventDefault();
            // Trigger the button element with a click
            document.querySelector('.lp-add-custom-tag').click();
            return false
        }
    });
...
function createFunnelNameValidation(){
    var response = true;
    var funnel_name = $("#create-funnel-pop #funnel_name").val().toLowerCase();
    if(funnel_name == "") {
...
function createFunnelButtonEnable(){
    let inputValue = $('.create-new-funnel-validate').map((i,el) => (el.value != '')?el.value:null).toArray();
     const funnel_name = $("#create-funnel").find('#funnel_name').val();
    if(inputValue.length === 2 && funnel_name.trim()){
