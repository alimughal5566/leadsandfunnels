...
(function (factory) {
    ...
    $.FE.PLUGINS.zalink = function (editor) {
        ...
        function _initInsertPopup () {
            ...
            $(".footer_extra_content").remove();
            // Set the template in the popup.
            var $popup = editor.popups.create('link.insert', template);

            $($popup).addClass('footer_extra_content');
            if (editor.$wp) {
            ...
            if (link) {
                ...
                if (!$current_image) {
                    ...
                    if(window.editor_type != 'cta'){
                        ...
                        cta_button = 'current';
                        for(var i=0; i <= $("#default_active #current").length; i++){
                            $($("#default_active #current")[i]).attr('id','');
                        }
                    }else{
                        anchor_id = 'cta_active';
                        cta_button = 'za_cta_style current';
                    }
                    if(editor.selection.text()) {
                        editor.format.apply("a", {href: href, target: _target, class: cta_button, id: anchor_id});
                    }else{
                        editor.html.insert('<a href="' + href + '" target="'+_target+'" class="'+cta_button+'" id="'+anchor_id+'">' + text + '</a>');
                    }

                    var currentLink = document.querySelector('.za_cta_style.current');
                    if(currentLink){
                        var parentNode = currentLink.parentNode;
                        var previousNode = currentLink.previousSibling;
                        if(previousNode){
                            if(!previousNode.textContent.length){
                                if(previousNode.nodeType == Node.TEXT_NODE){
                                    previousNode.textContent = previousNode.textContent + '\xa0'
                                } else {
                                    parentNode.insertBefore(document.createTextNode('\xa0'), currentLink)
                                }
                            }
                        } else {
                            parentNode.insertBefore(document.createTextNode('\xa0'), currentLink)
                        ...
                        if(nextNode){
                            if(!nextNode.textContent.length){
                                if(nextNode.nodeType == Node.TEXT_NODE){
                                    nextNode.textContent = '\xa0' + nextNode.textContent;
                                } else {
                                    parentNode.insertBefore(document.createTextNode('\xa0'), null)
                                }
                            }
                        } else {
                        ...
                        selection.selectAllChildren(currentLink);
                        selection.collapseToEnd();

                        $(document).trigger('lpFroalaEditorUpdateCurrentNode')
                    }
                    editor.selection.restore();
                }
                else {
...
{
    var el = '';
    el = document.querySelector(".current#cta_active");
    console.log(pos+' old position');
    if(!el){
        el = document.querySelector(".current#default_active");
        $($("#default_active")).css({'color':'#1daefc'});
    }

    if(pos < 0){
        pos = 0
    } else if( pos > el.innerText.length) {
