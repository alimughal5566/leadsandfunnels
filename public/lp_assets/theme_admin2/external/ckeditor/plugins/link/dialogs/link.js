		...
		contents: [
			...
			//		[ linkLang.toLeadpop, 'leadpop' ]
					...
					[
						{
							type : 'button',
							 id : 'browseInternal',
							 label : 'Link To leadPop',   
							 filebrowser :
							 {
								 action : 'Browse',
								 url : '/marketing/myleadpops'   
							 }
						 }
					]
				} ,				
				{
					...
					commit: function( data ) {
					    var theContact = this.getDialog().getContentElement( 'info', 'contactType' );
					    var theRel = this.getDialog().getContentElement( 'info', 'relType' );
						var dialog = CKEDITOR.dialog.getCurrent();
						if(theContact.getValue() == 'noform' && theRel.getValue() != 'noauto' ) {
						    var trel = 'lprel-' + theRel.getValue(); 
							dialog.setValueOf('advanced','advRel',trel);  // 
						    data.rel = 'lprel-' + trel;
						}
//						else 	if(theContact.getValue() == 'noform' && theRel.getValue() == 'noauto' ) {
	//						dialog.setValueOf('advanced','advRel','pete');  // 
		//				    data.rel = 'pete';
			//			}
//						alert(xtheContact.getValue()  + " -- " + xtheRel.getValue());
//alert(data.rel + ' popup');						
					}
				},
				{
					id: 'contactType',
					...
					items: [
						[ 'No Contact' , 'noform'],
						[ 'Name-Email-Phone-Question','name-email-phone-question' ], // name, email, phone,
						[ 'Name-Email-Phone-No-Question','name-email-phone-noquestion' ],
						[ 'Name-Email-No-Phone-Question','name-email-nophone-question' ],
						[ 'Name-Email-No-Phone-No-Question','name-email-nophone-noquestion' ],
						],
					onChange: function() {
					    var theRel = this.getDialog().getContentElement( 'info', 'relType' );
					    var theUrl = this.getDialog().getContentElement( 'info', 'url' );
					    var theProtocol = this.getDialog().getContentElement( 'info', 'protocol' );
					    var theContact = this.getDialog().getContentElement( 'info', 'contactType' );
                        
						if (theContact.getValue()  != 'noform') { // turn off auto popup
                            theUrl.setValue("leadpops-contact-form-in-use.com");
                            theProtocol.setValue("");
                            theRel.setValue('noauto');
						}
						else {
                            theUrl.setValue('');
                            theProtocol.setValue('');
					...
					commit: function( data ) {
					    var theContact = this.getDialog().getContentElement( 'info', 'contactType' );
					    var theRel = this.getDialog().getContentElement( 'info', 'relType' );
						if(theRel.getValue() == 'noauto' && theContact.getValue() != 'noform') {
						    data.rel = 'lpcontact-' + theContact.getValue();
						}
						else 	if(theContact.getValue() == 'noform' && theRel.getValue() == 'noauto' ) {
						    data.rel = 'gas';
						}
//						alert(ytheContact.getValue()  + " -- " + ytheRel.getValue());
//alert(data.rel + ' contact');
					}
				}
				]
			},
			...
			elements: [
				...
				children: [
					...
					'items': [
						...
						[ commonLang.targetLeadpop, '_leadpop'], 
						[ commonLang.targetContactform, '_contactform'] 
						],
					...
					[
						{
							type : 'button',
                             ...
                             {
								var dialog = CKEDITOR.dialog.getCurrent();
								var prandomid = dialog.getValueOf('advanced','advContentType');  // Populates the randomid in the Links dialogue.
								if (prandomid != "") {
                                    alert("Base CSS Class Previously Selected. Click 'Edit Button' To Modify.");								
									return false;
								}
							 
				                jQuery("#editflag").val("n");
                                var link = "https://myleads.leadpops.com/marketing/ctabuttonsck";
                                window.open(link , "_blank", "toolbar=no, scrollbars=no, resizable=yes, top=25, left=25, width=1200, height=700");								
							 },							 
							 /*
							 filebrowser :
							 {
								 action : 'Browse',
								 url : '/marketing/ctabuttonsck'   
							 }*/
						 }
					]
				} ,				
					...
					[
						{
                            ...
                            onClick: function()
                           {
								...
								var prandomid = dialog.getValueOf('advanced','advContentType');  // Populates the randomid in the Links dialogue.
								if (prandomid == "") {
                                    alert("Select Base CSS Class Prior To Editing.");								
									return false;
								}
				                jQuery("#editflag").val("y");
								var pclass = dialog.getValueOf('advanced','advCSSClasses');  // Populates the class field in the Links dialogue.
                                var page_id = jQuery("#page_id").val();
                                var guid = jQuery("#guid").val();
                                var link = "https://myleads.leadpops.com/marketing/editbuttons?c=" + pclass + "&p="+ page_id + "&g=" + guid + "&r=" + prandomid;
                                window.open(link , "_blank", "toolbar=no, scrollbars=no, resizable=yes, top=25, left=25, width=1200, height=700");								
							},							 
						 }
					]
...
//			this.setValueOf('advanced','advId',"hoss");  // Populates the randomid in the Links dialogue... hopefully
//			this.setValueOf('advanced','advId',"hoss");  // Populates the randomid in the Links dialogue... hopefully
//console.log(element);
//if (element) {
//var tt = element.getAttribute('data-id');
//alert(tt);
//}
		...
		onHide : function() {
		/*
            var editflag = jQuery("#editflag").val();
			if (editflag == "reload" || editflag == "y" || editflag == "n") {
		...
		onOk: function() {
			...
			// Compose the URL.
			// added shuey 2/12/2014
            var editflag = jQuery("#editflag").val();
			//alert(editflag);
			if (editflag == "y" && data.adv.advCSSClasses.length != 32) {
			    data.adv.advCSSClasses = data.adv.advCSSClasses + data.adv.advContentType;
			}
			else if (editflag == "reload") {  // change the buttons classes to be edited classes to include randomid
			    jQuery("a[class][type]").each(function (index) {
				    var cls = jQuery(this).attr("class");
				    var typ = jQuery(this).attr("type");
                    var newclass = "";
                    if (typeof cls !== "undefined" && typeof typ !== "undefined") {
					        if (cls.length == 16) {
							    jQuery(this).attr("class" , cls + typ);
							}
							else {
							    jQuery(this).attr("class" , cls);
							}
				      //      alert(cls + "  " + typ + " " + newclass);
							//data.adv.advCSSClasses = newclass;
				     }
				});
				if (data.adv.advCSSClasses.length != 32) {
					data.adv.advCSSClasses = data.adv.advCSSClasses + data.adv.advContentType;
				}
            }  
//alert("reload");
...
/*
				...
				$.ajax({
					url: "/marketing/refreshpagecontent",
					type: "post",
					data: "pageid=" + pageid,
					success : function(response) {
					alert(response);
						CKEDITOR.instances.mychild.setData(response);
						CKEDITOR.instances.mychild.resetDirty();
					},
					cache : false,
					async : false
				});
...
*/			
			...
			attributes.type = data.adv.advId;
//			alert(">"+ attributes.type + "<");
			...
			if ( data.adv ) {
				...
				jQuery("style[id=button-css-style]").remove();
			    jQuery("head").append("<style id='button-css-style' type='text/css'></style>"); 
			  
				jQuery.ajax( {
						type : "POST",
						url : "/marketing/getpageclasseseditor",
						data : "page_id=" + $("#page_id").val(),
						success : function(css) {
							jQuery('style[id="button-css-style"]').html(css);
						},
						cache : false,
						async : false
				});
				...
				var advAttr = function( inputName, attrName ) {
						...
						if ( value ) {
//						    alert(" yes " + inputName + " " + attrName + " " + value);
							attributes[ attrName ] = value;
						}	
						else {
					//	 no advId id
//						    alert(" no " +  inputName + " " + attrName );
                ...
                //this.setValueOf('advanced','advId',"moss");  // Populates the randomid in the Links dialogue... hopefully
//                this.setValueOf('advanced','advId',randomid);  // Populates the randomid in the Links dialogue... hopefully
				advAttr( 'advId', 'id' );
				advAttr( 'advLangDir', 'dir' );
				advAttr( 'advAccessKey', 'accessKey' );
				//attributes['data-id'] = randomid;
				//attributes['rel'] = randomid;
