...
//	console.log(editor);
		...
		return {
			...
			onShow: function() {
                ...
                if (!dialog.getValueOf('advanced','advId')) {  // if it is empty
                    dialog.setValueOf('advanced','advId',makeid());
                }				
						...
						[
							{
								type : 'button',
								 id : 'browseInternal',
								 label : 'Pop Page iFrames',   
//								 filebrowser :
	//							 {
		//							 action : 'Browse',
			//						 url : '/marketing/myiframes?frameid=' + this.getValueOf('advanced','advId')
				//				 },
                                 onClick : function() {			
									 var dialog = CKEDITOR.dialog.getCurrent();
									 var iframeId = dialog.getValueOf('advanced','advId');
									 var link = "/marketing/myiframes?frameid=" + iframeId;
									 window.open(link, "_blank", "toolbar=no, menubar=no, scrollbars=yes, resizable=yes, top=25, left=75, width=1250, height=700");								 
								 },
							 }
						]
					} ,				
