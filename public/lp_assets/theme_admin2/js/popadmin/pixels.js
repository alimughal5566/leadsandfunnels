...
function pixel_extra_fields___old(type, placement, elem){
    if(type == 2 && placement == 4){    // FACEBOOK_PIXELS   +   PIXEL_PLACEMENT_TYP
        $(".pixel_extra").hide();
        $("#pixel_other").val('');
        $(".pixel_position").hide();
    ...
    else if(type == 4 || type == 6){     // GOOGLE_CODE_CONVERSION_PIXEL
        $(".pixel_extra").hide(); // fb div
        $("#pixel_action").val('');
        $(".pixel_other").show(); //extra input
