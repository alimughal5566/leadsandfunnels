$(document).ready(function(){
  ...
  $(".bombbomb-wrapper img").mouseout(function() { $(".bombomb_desc").css('display','none'); });
    ...
    $("#_update_template_cta_btn").click(function(e){
      ...
      advancefooter_update($('.pfobtn').data('lpkeys'), false, radioValue);
      return;
    });

    $('#hideofooter:checkbox').click(function (){
        $(".save_option").trigger('click');
    });
...
function advancefooter_update(lpkeys,cta_update = true, defaultTplCtaMessage="n") {
      var client_id = $('#client_id').val();
      var akeys = lpkeys.split("~");
      var vertical_id = akeys[0];
      var subvertical_id = akeys[1];
...
function resetdefaultfooter(){
    ...
    $('.local-super-footer .lp-froala-textbox').froalaEditor('events.focus', false);
    $('#modal_reset_default').modal('toggle');
    $('#advance-footer .lp-footer-save .custom-btn-success button').trigger('click');
    goToByScroll("advanced-success-alert");
	$("#advanced-success-alert").fadeTo(2000, 500).slideUp(500, function(){
   		$(this).slideUp(500);
    });
