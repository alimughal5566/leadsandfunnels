...
var question_address_overlay = {

    overlay_select_list : [
        {selecter:".question-address-icon-type", parent:".question-address-input-icon-parent"},
        {selecter:".question-address-message", parent:".question-address-message-parent"},
        {selecter:".edit-content-address", parent:".edit-content-address-parent"},
    ],
    ...
    **/
    ...
    allCustomSelect: function () {
        var selectlist = question_address_overlay.overlay_select_list;
        for(var i = 0; i < selectlist.length; i++){
            question_address_overlay.initCustomSelect(selectlist[i].selecter,selectlist[i].parent);
        }
    },
    ...
    **/
    ...
    initCustomSelect: function (selecter,parent) {
        ...
        if(selectorClass == 'edit-content-address') {
            ...
            _selector.select2('val', $('.edit-content-address option:eq(1)').val());
        }
    },
  ...
  **/
   ...
   **/
    ...
    checkboxSlection: function () {
        jQuery('.state-all-checked').change(function () {
            if(this.checked){
                jQuery(this).parents('.select-state-modal').find('.state-checkbox').prop("checked", true);
                jQuery(this).parents('.select-state-modal').find('.state-save-btn').prop("disabled", false);
            }
            else {
                jQuery(this).parents('.select-state-modal').find('.state-checkbox').prop("checked", false);
                jQuery(this).parents('.select-state-modal').find('.state-save-btn').prop("disabled", true);
            }
        });

        jQuery('.state-reset-btn').click(function (e) {
            e.preventDefault();
            jQuery(this).parents('.select-state-modal').find('.state-checkbox').prop("checked", false);
            jQuery(this).parents('.select-state-modal').find('.state-all-checked').prop("checked", false);
            jQuery(this).parents('.select-state-modal').find('.state-save-btn').prop("disabled", true);
        });

        jQuery('.state-checkbox').change(function () {
            if (jQuery('.state-checkbox:checked').length <= 0) {
                jQuery(this).parents('.select-state-modal').find('.state-save-btn').prop("disabled", true);
            } else {
                jQuery(this).parents('.select-state-modal').find('.state-save-btn').prop("disabled", false);
            }
        ...
        jQuery('#select-state-modalselect-state-modal-alt').on('show.bs.modal', function (event) {
            jQuery('.state-checkbox').prop("checked", false);
            jQuery('.state-all-checked').prop("checked", false);
            jQuery('.state-save-btn').prop("disabled", true);
        });
    },
   ...
   **/
    dragable: function() {
        jQuery('.fb-options__clone').sortable({
   ...
   **/
    ...
    rangebar: function () {
        jQuery('#question-address-font-slider').bootstrapSlider({
            formatter: function(value) {
                jQuery('.question-address-font').val(value);
                jQuery(".cta-btn").css({
                    'font-size': value,
                });
                return   value +'px';
            ...
            min: 8,
            max: 40,
            value: jQuery('.question-address-font').val(),
            tooltip: 'always',
            tooltip_position: 'bottom'
        });

        jQuery('#question-address-icon-size-slider').bootstrapSlider({
            formatter: function(value) {
                jQuery('.question-address-icon-size').val(value);
            ...
            min: 10,
            max: 28,
            value: jQuery('.question-address-icon-size').val(),
            tooltip: 'always',
            tooltip_position: 'bottom'
        });
    ...
    **/
    ...
    color_picker: function () {
        ...
        $('#question-address-clr-icon').click(function () {
            var name = ".question-address-icon-clr";
        ...
        $('#question-address-icon-clr').ColorPicker({
            color: "#ffffff",
            flat: true,
            opacity:true,
            width: 203,
            ...
            onChange: function (hsb, hex, rgb, rgba) {
                ...
                $(".question-address-icon-clr .color-box__r .color-box__rgb").val(rgb.r);
                $(".question-address-icon-clr .color-box__g .color-box__rgb").val(rgb.g);
                $(".question-address-icon-clr .color-box__b .color-box__rgb").val(rgb.b);
                $(".question-address-icon-clr .color-box__hex-block").val('#'+hex);
                $('#question-address-clr-icon').find('.last-selected__box').css('backgroundColor', rgba_fn);
                $('#question-address-clr-icon').find('.last-selected__code').text('#'+hex);
   ...
   * */
    ...
    **  Address Required
    ...
    editAddress: function () {
        jQuery('[name="address_required"]').change(function(){
            if (jQuery(this).is(':checked')) {
                console.log('checked');
                jQuery('.edit-content-address-parent').tooltipster('disable');
                jQuery('.edit-content-address-parent').removeClass('select2-parent_disabled');
            }else {
                console.log('un-checked');
                jQuery('.edit-content-address-parent').tooltipster('enable');
                jQuery('.edit-content-address-parent').addClass('select2-parent_disabled');
            }
        });
    },
 ...
 * */
    ...
    **/
    ...
    init: function() {
        ...
        question_address_overlay.openclose();
        question_address_overlay.customscroll();
        question_address_overlay.selecticon();
        question_address_overlay.rangebar();
        question_address_overlay.color_picker();
        question_address_overlay.allCustomSelect();
        question_address_overlay.outsideclick();
        question_address_overlay.menugroup();
        question_address_overlay.color_picker_dropdown();
        question_address_overlay.quick_access();
        question_address_overlay.dragable();
        question_address_overlay.tooltip();
        question_address_overlay.outside_hover();
        question_address_overlay.editAddress();
        question_address_overlay.checkboxSlection();
        question_address_overlay.farola_editor();
    },
};

jQuery(document).ready(function() {
    question_address_overlay.init();
});
