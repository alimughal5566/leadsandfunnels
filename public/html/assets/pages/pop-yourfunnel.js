...
$(document).ready(function() {
    ...
    $('#ex01').bootstrapSlider({
        formatter: function(value) {
            $('#lightbox-Background').val(value);
            return   value +'%';
        },
        min: 0,
        max: 100,
        value: $('#lightbox-Background').val(),
        tooltip: 'always',
    ...
    $('#ex05').bootstrapSlider({
        formatter: function(value) {
            $('#button-fontsize').val(value);
            return   value +'px';
        },
        min: 8,
        max: 50,
        value: $('#button-fontsize').val(),
        tooltip: 'always',
        tooltip_position:'bottom'
    });
    ...
    $('#button-text-font-family').select2({
        // minimumResultsForSearch: -1,
        //placeholder: 'Open sans',
        width: '100%', // need to override the changed default
        dropdownParent: $('.select2__parent-button-text-font-type'),
        templateResult: function (data, container) {
            if (data.element) {
                var font_family = $(data.element).css("font-family")
                $(container).css('font-family', font_family );
        ...
        jQuery('.select2__parent-button-text-font-type .select2-selection__rendered').css('opacity', '0');
    }).on('select2:open', function() {
        $(".select2-search__field").attr("placeholder", "Search....");
        jQuery('.select2__parent-button-text-font-type .select2-results__options').css('pointer-events', 'none');
        setTimeout(function() {
            jQuery('.select2__parent-button-text-font-type .select2-results__options').css('pointer-events', 'auto');
        }, 300);
        jQuery('.select2__parent-button-text-font-type .select2-dropdown').hide();
        jQuery('.select2__parent-button-text-font-type .select2-dropdown').css({'display': 'block', 'opacity': '1', 'transform': 'scale(1, 1)'});
        //jQuery('.select2__parent-button-text-font-type .select2-selection__rendered').hide();
        lpUtilities.niceScroll();
        setTimeout(function () {
            jQuery('.select2__parent-button-text-font-type .select2-dropdown .nicescroll-rails-vr').each(function () {
                this.style.setProperty( 'opacity', '1', 'important' );
                var getindex = jQuery('#button-text-font-family').find(':selected').index();
                var defaultHeight = 44;
                var scrolledArea = getindex * defaultHeight - 50;
                $(".select2-results__options").getNiceScroll(0).doScrollTop(scrolledArea);
                this.style.setProperty( 'opacity', '1', 'important' );
        ...
        if(!amIclosing) {
            e.preventDefault();
            amIclosing = true;
            jQuery('.select2__parent-button-text-font-type .select2-dropdown').attr('style', '');
            setTimeout(function () {
                jQuery('#button-text-font-family').select2("close");
            }, 200);
        } else {
            amIclosing = false;
        }
    }).on('select2:close', function() {
        jQuery('.select2__parent-text-button-font-type .select2-selection__rendered').show();
        jQuery('.select2__parent-text-button-font-type .select2-results__options').css('pointer-events', 'none');
    });

    $('.select2js__open-popun').select2({
        minimumResultsForSearch: -1,
        // width: '230px',
        width: '100%',
        dropdownParent: $('.select2js__open-popun-parent')
        }).on('select2:openning', function() {
            jQuery('.select2js__open-popun-parent .select2-selection__rendered').css('opacity', '0');
        }).on('select2:open', function() {
            jQuery('.select2js__open-popun-parent .select2-results__options').css('pointer-events', 'none');
            setTimeout(function() {
                jQuery('.select2js__open-popun-parent .select2-results__options').css('pointer-events', 'auto');
            }, 300);
            jQuery('.select2js__open-popun-parent .select2-dropdown').hide();
            jQuery('.select2js__open-popun-parent .select2-dropdown').css({'display': 'block', 'opacity': '1', 'transform': 'scale(1, 1)'});
            jQuery('.select2js__open-popun-parent .select2-selection__rendered').hide();
        }).on('select2:closing', function(e) {
            if(!amIclosing) {
                e.preventDefault();
                amIclosing = true;
                jQuery('.select2js__open-popun-parent .select2-dropdown').attr('style', '');
                setTimeout(function () {
                    jQuery('.select2js__open-popun').select2("close");
                }, 200);
            } else {
                amIclosing = false;
            }
        }).on('select2:close', function() {
            jQuery('.select2js__open-popun-parent .select2-selection__rendered').show();
            jQuery('.select2js__open-popun-parent .select2-results__options').css('pointer-events', 'none');
        }).on('change', function () {
        if($(this).val() == "nt"){
            $('.basic-setting').addClass('disabled');
        }else {
            $('.basic-setting').removeClass('disabled');

        }
    });

    var nightmode_options = [
        {
            id:0,
            text:'<div class="mode-option center">Center</div>'
        },
        {
            id:1,
            text:'<div class="mode-option left">Left Drawer</div>'
        },
        {
            id:1,
            text:'<div class="mode-option right">Right Drawer</div>'
        }
    ];

    $('.select2js__lightbox-mode').select2({
        width: '100%',
        data: nightmode_options,
        minimumResultsForSearch: -1,
        selectionTitleAttribute: false,
        templateResult: function (d) { return $(d.text); },
        templateSelection: function (d) { return $(d.text); },
        dropdownParent: $('.select2js__lightbox-mode-parent')
    }).on('select2:openning', function() {
        jQuery('.select2js__lightbox-mode-parent .select2-selection__rendered').css('opacity', '0');
    }).on('select2:open', function() {
        jQuery('.select2js__lightbox-mode-parent .select2-results__options').css('pointer-events', 'none');
        setTimeout(function() {
            jQuery('.select2js__lightbox-mode-parent .select2-results__options').css('pointer-events', 'auto');
        }, 300);
        jQuery('.select2js__lightbox-mode-parent .select2-dropdown').hide();
        jQuery('.select2js__lightbox-mode-parent .select2-dropdown').css({'display': 'block', 'opacity': '1', 'transform': 'scale(1, 1)'});
        jQuery('.select2js__lightbox-mode-parent .select2-selection__rendered').hide();
    }).on('select2:closing', function(e) {
        if(!amIclosing) {
            e.preventDefault();
            amIclosing = true;
            jQuery('.select2js__lightbox-mode-parent .select2-dropdown').attr('style', '');
            setTimeout(function () {
                jQuery('.select2js__lightbox-mode').select2("close");
            }, 200);
        } else {
            amIclosing = false;
        }
    }).on('select2:close', function() {
        jQuery('.select2js__lightbox-mode-parent .select2-selection__rendered').show();
        jQuery('.select2js__lightbox-mode-parent .select2-results__options').css('pointer-events', 'none');
    });
    $('.basic-setting').hover(function (e) {
    ...
    $('.select2js__width-unit').select2({
        minimumResultsForSearch: -1,
        width: '87px', // need to override the changed default
        dropdownParent: $('.select2js__width-unit-parent')
    }).on('select2:openning', function() {
        jQuery('.select2js__width-unit-parent .select2-selection__rendered').css('opacity', '0');
    }).on('select2:open', function() {
        jQuery('.select2js__width-unit-parent .select2-results__options').css('pointer-events', 'none');
        setTimeout(function() {
            jQuery('.select2js__width-unit-parent .select2-results__options').css('pointer-events', 'auto');
        }, 300);
        jQuery('.select2js__width-unit-parent .select2-dropdown').hide();
        jQuery('.select2js__width-unit-parent .select2-dropdown').css({'display': 'block', 'opacity': '1', 'transform': 'scale(1, 1)'});
        jQuery('.select2js__width-unit-parent .select2-selection__rendered').hide();
    }).on('select2:closing', function(e) {
        if(!amIclosing) {
            e.preventDefault();
            amIclosing = true;
            jQuery('.select2js__width-unit-parent .select2-dropdown').attr('style', '');
            setTimeout(function () {
                jQuery('.select2js__width-unit').select2("close");
            }, 200);
        } else {
            amIclosing = false;
        }
    }).on('select2:close', function() {
        jQuery('.select2js__width-unit-parent .select2-selection__rendered').show();
        jQuery('.select2js__width-unit-parent .select2-results__options').css('pointer-events', 'none');
    });

    $('.select2js__height-unit').select2({
        minimumResultsForSearch: -1,
        width: '87px', // need to override the changed default
        dropdownParent: $('.select2js__height-unit-parent')
    }).on('select2:openning', function() {
        jQuery('.select2js__height-unit-parent .select2-selection__rendered').css('opacity', '0');
    }).on('select2:open', function() {
        jQuery('.select2js__height-unit-parent .select2-results__options').css('pointer-events', 'none');
        setTimeout(function() {
            jQuery('.select2js__height-unit-parent .select2-results__options').css('pointer-events', 'auto');
        }, 300);
        jQuery('.select2js__height-unit-parent .select2-dropdown').hide();
        jQuery('.select2js__height-unit-parent .select2-dropdown').css({'display': 'block', 'opacity': '1', 'transform': 'scale(1, 1)'});
        jQuery('.select2js__height-unit-parent .select2-selection__rendered').hide();
    }).on('select2:closing', function(e) {
        if(!amIclosing) {
            e.preventDefault();
            amIclosing = true;
            jQuery('.select2js__height-unit-parent .select2-dropdown').attr('style', '');
            setTimeout(function () {
                jQuery('.select2js__height-unit').select2("close");
            }, 200);
        } else {
            amIclosing = false;
        }
    }).on('select2:close', function() {
        jQuery('.select2js__height-unit-parent .select2-selection__rendered').show();
        jQuery('.select2js__height-unit-parent .select2-results__options').css('pointer-events', 'none');
    });

    $('#launching-delay').select2({
        minimumResultsForSearch: -1,
        width: '100%', // need to override the changed default
        dropdownParent: $('.launching-delay-parent')
    }).on('select2:openning', function() {
        jQuery('.launching-delay-parent .select2-selection__rendered').css('opacity', '0');
    }).on('select2:open', function() {
        jQuery('.launching-delay-parent .select2-results__options').css('pointer-events', 'none');
        setTimeout(function() {
            jQuery('.launching-delay-parent .select2-results__options').css('pointer-events', 'auto');
        }, 300);
        jQuery('.launching-delay-parent .select2-dropdown').hide();
        jQuery('.launching-delay-parent .select2-dropdown').css({'display': 'block', 'opacity': '1', 'transform': 'scale(1, 1)'});
        jQuery('.launching-delay-parent .select2-selection__rendered').hide();
    }).on('select2:closing', function(e) {
        if(!amIclosing) {
            e.preventDefault();
            amIclosing = true;
            jQuery('.launching-delay-parent .select2-dropdown').attr('style', '');
            setTimeout(function () {
                jQuery('#launching-delay').select2("close");
            }, 200);
        } else {
            amIclosing = false;
        }
    }).on('select2:close', function() {
        jQuery('.launching-delay-parent .select2-selection__rendered').show();
        jQuery('.launching-delay-parent .select2-results__options').css('pointer-events', 'none');
    });

    $('#closing-delay').select2({
        minimumResultsForSearch: -1,
        width: '100%', // need to override the changed default
        dropdownParent: $('.closing-delay-parent')
    }).on('select2:openning', function() {
        jQuery('.closing-delay-parent .select2-selection__rendered').css('opacity', '0');
    }).on('select2:open', function() {
        jQuery('.closing-delay-parent .select2-results__options').css('pointer-events', 'none');
        setTimeout(function() {
            jQuery('.closing-delay-parent .select2-results__options').css('pointer-events', 'auto');
        }, 300);
        jQuery('.closing-delay-parent .select2-dropdown').hide();
        jQuery('.closing-delay-parent .select2-dropdown').css({'display': 'block', 'opacity': '1', 'transform': 'scale(1, 1)'});
        jQuery('.closing-delay-parent .select2-selection__rendered').hide();
    }).on('select2:closing', function(e) {
        if(!amIclosing) {
            e.preventDefault();
            amIclosing = true;
            jQuery('.closing-delay-parent .select2-dropdown').attr('style', '');
            setTimeout(function () {
                jQuery('#closing-delay').select2("close");
            }, 200);
        } else {
            amIclosing = false;
        }
    }).on('select2:close', function() {
        jQuery('.closing-delay-parent .select2-selection__rendered').show();
        jQuery('.closing-delay-parent .select2-results__options').css('pointer-events', 'none');
    });
    ...
    $('#popup-open').select2({
        minimumResultsForSearch: -1,
        width: '100%', // need to override the changed default
        dropdownParent: $('.popup-open-parent')
    }).on('change',function () {
        if ($(this).val() == 1) {
            $('.window-option-slide').slideDown();
        }else {
            $('.window-option-slide').slideUp();
        }
    }).on('select2:openning', function() {
        jQuery('.popup-open-parent .select2-selection__rendered').css('opacity', '0');
    }).on('select2:open', function() {
        jQuery('.popup-open-parent .select2-results__options').css('pointer-events', 'none');
        setTimeout(function() {
            jQuery('.popup-open-parent .select2-results__options').css('pointer-events', 'auto');
        }, 300);
        jQuery('.popup-open-parent .select2-dropdown').hide();
        jQuery('.popup-open-parent .select2-dropdown').css({'display': 'block', 'opacity': '1', 'transform': 'scale(1, 1)'});
        jQuery('.popup-open-parent .select2-selection__rendered').hide();
    }).on('select2:closing', function(e) {
        if(!amIclosing) {
            e.preventDefault();
            amIclosing = true;
            jQuery('.popup-open-parent .select2-dropdown').attr('style', '');
            setTimeout(function () {
                jQuery('#popup-open').select2("close");
            }, 200);
        } else {
            amIclosing = false;
        }
    }).on('select2:close', function() {
        jQuery('.popup-open-parent .select2-selection__rendered').show();
        jQuery('.popup-open-parent .select2-results__options').css('pointer-events', 'none');
    });
